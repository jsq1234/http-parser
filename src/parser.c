...
int parse_http(char* msg, http_request_t* request){
    ...
    size_t left = strlen(msg); // the size of remaining string when we have traversed it and at a position, say i.
  
    if( msg_len < 4 ){ // if the msg len is less than 4, we can be sure that it doesn't have \r\n\r\n at the end
    ...
    CHECK_ERR(method_valid,ERR_METH)
    CHECK_SPACE(itr1,left, ERR_REQ) // if there is no space, throw an error and return -1
    ...
    CHECK_NULL(itr2,ERR_SIZE) // check if we have exhausted our string. Return error if we have. 
    CHECK_SPACE(itr2,left,ERR_REQ);
    ...
    int valid_version = 0;
    if( safe_strncmp(itr1,"HTTP/1.0",left,8) == 1 || safe_strncmp(itr1, "HTTP/1.1",left,8) == 1 ){
        valid_version = 1;
    ...
    int by = 0;
    while(1){
        by = parse_headers(itr1,left,request);
        MOVE(itr1,left,by)
...
int parse_field(char* itr1, size_t left, string_t* header){
    ...
    CHECK_CRLF(itr1,left,ERR_CRLF) // if it doesn't end with \r\n, throw error and return
...
int parse_headers(char* itr1, size_t left, http_request_t* request){
    ...
    int by = 0;
    if( safe_strncmpcase(itr1,"Host:", left, 5) ){
        MOVE(itr1,left,5)
        //printf("it is host!\n");
        by = parse_field(itr1,left,&request->headers.host);
        by = (by >= 0) ? by + 5 : -1;
        
    }else if( safe_strncmpcase(itr1,"Connection:",left,11)  ){
        MOVE(itr1,left,11)
        by = parse_field(itr1,left,&request->headers.connection);
        by = (by >= 0) ? by + 11 : -1;
    }else if( safe_strncmpcase(itr1,"Content-Type:",left,13) ){
        MOVE(itr1,left,13)
        by = parse_field(itr1,left,&request->headers.content_type);
        by = (by >= 0) ? by + 13 : -1;
    }else if( safe_strncmpcase(itr1,"Content-Length:",left,15) ){
        MOVE(itr1,left,15)
